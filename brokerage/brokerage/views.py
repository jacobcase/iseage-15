...
def user():
    ...
    stock_holds = StockHolder.query.filter_by(user_id=mod_user.id).all()
    stock_holdings = []
...
def home_page():
    ...
    stock_holds = StockHolder.query.filter_by(user_id=user.id).all()
...
def stock_edit():
    ...
    if request.method == 'POST':
        ...
        DB.session.commit()
    stock = Stock.query.filter_by(name=stock_name).first()   
    return render_template("stock.html", stock=stock, admin=user.isAdmin())
